...
| 配置 | 内存 (RAM) | 显存 (VRAM) | 说明 |
|------|-----------|-------------|------|
| **GPU训练 (推荐)** | 8GB | 4-6GB | 默认配置 |
| **GPU训练 (小batch)** | 8GB | 2-4GB | `BATCH_SIZE=4` |
| **CPU训练** | 16GB | 无需 | 速度慢，但能跑 |
| **CPU训练 (最低)** | 8GB | 无需 | `BATCH_SIZE=4` |

### 常见显卡参考

| 显卡 | 显存 | 能否训练 |
|------|------|---------|
| RTX 4090/3090 | 24GB | ✅ 轻松 |
| RTX 4080/3080 | 10-16GB | ✅ 轻松 |
| RTX 4060/3060 | 8-12GB | ✅ 可以 |
| RTX 3050/2060 | 6-8GB | ✅ 改小batch |
| GTX 1660/1650 | 4-6GB | ⚠️ 勉强，batch=4 |
| 集成显卡 | - | ❌ 用CPU |
...
```python
BATCH_SIZE = 4   # 从16改为4
MAX_LENGTH = 32  # 从64改为32
```

这样可以把显存需求降到 **2-3GB**，内存需求降到 **6-8GB**。
...
|------|---------|------|
| RTX 4090 | 1-2 分钟 | 最快 |
| RTX 4080/3080 | 2-3 分钟 | 很快 |
| RTX 4060/3060 | 3-5 分钟 | 推荐 |
| RTX 3050/2060 | 5-8 分钟 | 可接受 |
| GTX 1660/1650 | 8-15 分钟 | 较慢 |
| CPU (i7/R7) | 30-60 分钟 | 慢，但能跑 |
| CPU (i5/R5) | 60-90 分钟 | 建议减少数据量 |

> 💡 首次运行需要下载预训练模型 (~400MB)，下载时间取决于网络速度
